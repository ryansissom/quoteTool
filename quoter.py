...
def open_matcher():
    ...
    entry_label = ttk.Label(top_frame, text="Customer Description:")
    ...
    search_button = ttk.Button(button_frame, text="Search", command=lambda: fuzzymatch(entry.get(), manufacturer_dropdown.get(), store_name_dropdown.get(), tree1))
    search_button.pack(side='left', padx=5)

    # Add a "Custom Entry" button to the button frame
    custom_entry_button = ttk.Button(button_frame, text="Custom Entry", command=lambda: open_custom_entry_window(tree2, entry.get()))
    ...
    add_to_quote_button = ttk.Button(button_frame, text="Add to Quote", command=lambda: add_to_quote(entry.get(), quantity_entry.get(), margin_entry.get(), tree2))
